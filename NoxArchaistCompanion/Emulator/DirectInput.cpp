...
namespace DIMouse
{
	...
	{
		LogFileOutput(L"DirectInputInit: g_bDisableDirectInput=%d\n", g_bDisableDirectInput);
		...
		LogFileOutput(L"DirectInputInit: DirectInputUninit()\n");
		...
		LogFileOutput(L"DirectInputInit: DirectInput8Create(), hr=0x%08X\n", hr);
		if (FAILED(hr))
			return hr;

		// Obtain an interface to the system mouse device.
		hr = g_pDI->CreateDevice( GUID_SysMouse, &g_pMouse, NULL );
		LogFileOutput(L"DirectInputInit: CreateDevice(), hr=0x%08X\n", hr);
		if (FAILED(hr))
			return hr;
		...
		LogFileOutput(L"DirectInputInit: SetDataFormat(), hr=0x%08X\n", hr);
		if (FAILED(hr))
			return hr;
		...
		LogFileOutput(L"DirectInputInit: SetCooperativeLevel(), hr=0x%08X\n", hr);
		if( hr == DIERR_UNSUPPORTED && !bForeground && bExclusive )
		{
			DirectInputUninit(hDlg);
			LogFileOutput(L"DirectInputInit: DirectInputUninit()n");
		...
		if( !bImmediate )
		{
			...
			LogFileOutput(L"DirectInputInit: SetProperty(), hr=0x%08X\n", hr);
			if (FAILED(hr))
				return hr;
		}
		...
		hr = g_pMouse->Acquire();
		LogFileOutput(L"DirectInputInit: Acquire(), hr=0x%08X\n", hr);
		if (FAILED(hr))
			return hr;
		...
		LogFileOutput(L"DirectInputInit: SetTimer(), id=0x%08X\n", g_TimerIDEvent);
		if (g_TimerIDEvent == 0)
			return E_FAIL;
	...
	{
		LogFileOutput(L"DirectInputUninit\n");
		...
		if( g_pMouse ) 
		{
			...
			LogFileOutput(L"DirectInputUninit: Unacquire(), hr=0x%08X\n", hr);
		}
		...
		if (g_TimerIDEvent)
		{
			...
			LogFileOutput(L"DirectInputUninit: KillTimer(), res=%d\n", bRes ? 1 : 0);
			g_TimerIDEvent = 0;
		}
	}
